...
{
	maquina_estado_rebotes(); //Como este timer se ocupa de los tiempos de los rebotes,debemos llamar a la funcion que gestiona los rebotes cuando se activa este timer
...
void timer0_trp() {
	...
	rTCON ^= 0x3; //Ponemos este número en vez de 0x09 para no modificar los demás timers de la placa (Este timer no tiene auto-reload)
}

void timer0_boton_mantenido(){ //Debe ser autoreload para en caso de seguir manteniendose, volver a saltar el timer
	rTCNTB0 = 3200; //50 ms
	//Ponemos manual update del timer 2 a 1 (bit 13)
	rTCON |=0x2;
	rTCON ^= 0x1;
