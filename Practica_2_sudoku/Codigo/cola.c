...
// Apilar elementos en la pila, en direccion full descending
void cola_guardar_eventos(uint8_t ID_evento, uint32_t auxData)
{
	int tiempo_evento = timer1_count();
	uint32_t dato_evento = (uint32_t) ID_evento << 24;//dato_evento era un entero de 8 bits, debemos pasarlo a 32 bits
	                                                  //para que se almacene alineado en la cola
	...
	// Apilamos los datos
	 ...
	 cola[++iterador] = tiempo_evento; // Ponemos ++iterador y no iterador++ para que aumente antes de acceder a la posicion
...
void cola_guardar_eventos_prueba(uint32_t tiempo)
{
	// Si la cola llega al limite se reinicia
	if(iterador == ESPACIO_MAX) {
		iterador = 1;
