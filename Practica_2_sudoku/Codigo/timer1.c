...
{
	timer1_num_int ++; //Incrementamos la variable del contador para saber que se ha producido una excepcion más
	//switch_leds = 1;   En este caso no necesitamos cambiar switch_leds para que se iluminen
	// borrar bit en I_ISPC para desactivar la solicitud de interrupción
	rI_ISPC |= BIT_TIMER1; // BIT_TIMER1 está definido en 44b.h y pone un uno en el bit 12 que correponde al Timer1
}
...
void timer1_init(void)
{
	...
	rINTMSK &= ~(BIT_TIMER1); // habilitamos en vector de mascaras de interrupcion el Timer1 (bits 26 y 12, BIT_GLOBAL y BIT_TIMER1 están definidos en 44b.h)
...
void timer1_start()
{
	rTCNTO1 = rTCNTB1; // Inicializamos rTCNTO1 con el valor inicial de cuenta (Reiniciamos la cuenta)
