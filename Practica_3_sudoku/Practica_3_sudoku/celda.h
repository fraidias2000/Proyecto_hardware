...
__inline uint8_t valor_es_pista(CELDA *celdaptr, uint8_t valor){
	uint8_t no_es_pista = 0;
	uint8_t es_pista = 1;
	uint8_t valor_celda =  celda_leer_valor(celdaptr);
	if(valor_celda == valor){ // Comprobamos si el valor introducido
		                      // por el usuario es el mismo que el de la celda
...
{
	if((*celdaptr & (0x0001 <<	 (BIT_CANDIDATOS + valor - 1) )) == 0){ //Si es candidato
    	return 1;
    }
    return 0;
}

__inline int obtener_candidatos(CELDA celda){
	return (celda & 0xFF80);
}
